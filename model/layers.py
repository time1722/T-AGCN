...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model, max_len=5000):
        ...
        pe = pe.unsqueeze(0).transpose(0, 1)  # 调整为 (max_len, 1, d_model)
...
class TransformerBlock(nn.Module):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1):
        ...
        self.multi_head_attention = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
...
class TemporalConvLayer(nn.Module):
    ...
    def __init__(self, Kt, c_in, c_out, n_vertex, act_func):
        ...
        self.position_encoder = PositionalEncoding(d_model=c_out)
        self.linear = nn.Linear(in_features=c_out * n_vertex, out_features=c_out)
    ...
    def forward(self, x):
        ...
        batch_size, d_model, timestep, n_vertex = x_causal_conv.shape
        x_position_in = x_causal_conv.reshape(timestep, -1 ,d_model)
        x_position_out = self.position_encoder(x_position_in)

        x_transformer_out = self.transformer(x_position_out)

        x = x_transformer_out.reshape(batch_size, d_model, timestep, n_vertex)
